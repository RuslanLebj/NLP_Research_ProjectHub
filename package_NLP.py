...
def dock_processing(url_dock_address):
    ...
    with open('stopwords/russian_stopwords.pkl', 'rb') as file:
    ...
    with open("test_set_names_ner.txt", "w") as file:
        file.write(preprocessed_title_page_text)
    ...
    technology_list = []
    technology_extractor = spacy.load("ru_core_news_technologies_md")  # загрузка модели
    for task in tasks_list:
        doc = technology_extractor(task)
        for entity in doc.ents:
            if entity.label_ == "TECHNOLOGY":  # фильтрация сущностей, относящихся к продуктам/технологиям
